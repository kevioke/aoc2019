...
%halt instruction
run_computer(State, InstPtr, State, [State], _, []) :- nth0(InstPtr, State, 99).

%add or multiply instructions
run_computer(State, InstPtr, FinalState, [State|NextStates], Inputs, Outputs) :-
  ...
  run_computer(NextState, NextInstPtr, FinalState, NextStates, Inputs, Outputs).

%print instruction
run_computer(State, InstPtr, FinalState, [printed(State, InstPtr)|NextStates], Inputs, [Val|Outputs]) :-
  nth0(InstPtr, State, CurrentInt),
  OpInt is mod(CurrentInt, 100),
  OpInt = 4,
  ...
  run_computer(State, NextInstPtr, FinalState, NextStates, Inputs, Outputs).

%read instruction
run_computer(State, InstPtr, FinalState, [State|NextStates], [Input|Inputs], Outputs) :-
  nth0(InstPtr, State, CurrentInt),
  OpInt is mod(CurrentInt, 100),
  OpInt = 3,
  ...
  resolve_val(State, immediate_mode, DestPtr, Dest),
  replace(State, Dest, Input, NextState), %Hard coded input!!!
...
run_amps(State, [Phase], Input, Output) :-
  run_computer(State, 0, _, _, [Phase|Input], Output).

run_amps(State, [Phase|Phases], Input, Output) :-
  run_computer(State, 0, _, _, [Phase|Input], IntermediateOutput),
...
run_amps_feedback([State|States], [Phase|Phases], Input, Output, StopPhase) :-
  run_computer(State, 0, _, _, [Phase|Input], IntermediateOutput),

run_amps_feedback([State|States], [Phase|Phases], Input, Output, StopPhase) :-
  run_computer(State, 0, _, _, [Phase|Input], IntermediateOutput),
  append(States, State, NewStates),
